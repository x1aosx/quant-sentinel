...
- 市场情报：支持可配置 RSS/Atom 新闻、政策和公告源；执行清洗、三级去重、事件聚合、结构化影响分析、主题热度统计、盘前快报与重要事件通知记录。
- 机会发现：基于主题、事件、价格动量、波动、流动性和拥挤度构建可解释 DiscoveryScore，按横截面排名生成 Watch/Focus 候选池，每个候选保留分数贡献、原因、风险和模型版本。
- 支撑阻力：融合成交密度、已确认 swing pivots、ATR 区间与更长周期共振。
- 价格行为：本地计算趋势背景、波段结构、突破质量、H/L 计数、支撑阻力与保守决策参考。
- 图表：K线与关键价位区间联动展示。
- AI 分析：两阶段市场诊断与决策、结果持久化与历史回看、决策路径可视化、未来走势与下一根K线预期、原始响应和调试信息。
- 实时盯盘：数据集标的与自定义标的、24小时/A股交易时段/自定义调度、状态与最近结果查看、并发批量和飞书通知。
- 定时任务：项目级 Scheduler 基础设施，支持 Local/Redis 调度、Planner 批量 DAG、优先级队列、重试、超时、分布式锁、Worker 心跳、强制取消与执行历史；任务中心提供计划、执行和 Worker 管理。
- Alpha 因子研究中心：版本化因子词表、StackVM、约束公式采样、Walk-Forward 因子训练、策略资产、A 股成本回测、稳健性审计与已收盘 K 线实时信号；训练、回测和实时共用同一信号核。
